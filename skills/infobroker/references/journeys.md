...
| Check provider status | `inspect_providers` (list / health) | list state (incl. inactivity reason) → live health per slug |
| Knowledge base search | `manage_kb` (search) | vector + keyword hybrid → freshness-adjusted score; pass `collection: "reports"` to recall stored reports (search/list default to the `default` collection) |
| Store a report | `manage_kb` (ingest `source_type: report`) | index → `reports` collection → revisit via list/get with `collection: "reports"` |
